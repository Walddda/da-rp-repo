    ...
    <div class="mx-auto px-2">
        <div class="relative flex justify-between h-32 pt-8">
            ...
            <div class="flex flex-col items-center space-x-4 pl-5" :class="[showNoti ? 'h-32' : 'h-16']" v-if="$page.props.auth.user">
                <div v-if="profile">
                    <span class="nav-link" @click="showNoti = !showNoti">
                        <i class="fa fa-bell" id="notify_icon"></i>
                        <span id="notifiy_num" v-show="unreadNoti > 0"> {{unreadNoti}}</span>
                    </span>                    
                ...
                <div v-else >
                    ...
                    <span class="nav-link pl-10 inline" @click="toggleSearch" :style="[this.backgroundOp <= 0.9 ? {display: 'none'} : this.backgroundOp <= 1 ?  {opacity: (-4+(this.backgroundOp/0.2)), display: 'inline'} : {display:'inline'}]">Search</span>
                    <notification @unreadCount="(n) => unreadNoti = n" v-show="showNoti"></notification>
                    <div v-if="showDrop" class="absolute w-40 bg-opacity-0" @mouseleave="mouseLeavesMeth">
